...
本文档描述 OpenClaw Hub 的完整产品规划，分为三个阶段：

| 阶段 | 状态 | 技术方案 | 主要功能 |
|------|------|---------|---------|
| **Phase 1** | [已实现] | 静态站点 (SSG) | Skill 市场、教程中心、配置展示、本地存储 |
| **Phase 2** | [规划中] | 动态升级 (SSR) | 用户系统、评分评论、收藏同步 |
| **Phase 3** | [未来] | 完整平台 | 社区论坛、管理后台、付费功能 |

**当前状态**：Phase 1 已完成，200+ Skills、30+ 教程已上线。
**技术栈**：Next.js 16 + React 19 + TypeScript + Tailwind CSS 3.4.17
...
**数据文件结构**：
- `src/data/skills.json` - 200+ Skills，Git 版本控制
- `src/data/tutorials.json` - 30+ 教程，完整 Markdown
- `src/data/configs.json` - 配置分享，结构化数据
...
### 3.2 🎨 Agent 配置分享站 [Phase 1 - 基础实现]

> 当前为静态展示，上传/编辑功能将在 Phase 2 实现。
...
### 3.3 📖 文档中心 [Phase 1 - 已实现]

> 已实现为教程中心，30+ 篇完整 Markdown 教程。
...
### 3.4 💬 社区论坛 [Phase 3 - 规划中]

> **当前阶段**：Phase 1 通过外部社群（微信群/Discord）进行用户交流。
> 完整论坛功能将在 Phase 3 实现。
...
### 3.5 👤 用户体系 [Phase 2 - 规划中]

> **Phase 1 替代方案**：使用 LocalStorage 实现收藏夹、阅读进度、主题设置，无需登录即可使用。
...
### 3.6 🔧 管理后台 [Phase 3 - 规划中]

> **Phase 1 方案**：直接编辑 JSON 文件，Git 版本控制。
...
┌─────────────────────────────────────────────────────────┐
│                       前端层                            │
│  Next.js 16 + React 19 + TypeScript + Tailwind CSS 3.4.17 │
...
> **说明**：本节描述 Phase 2 及以后的数据库设计。Phase 1 使用 JSON 文件存储数据。
...
> **说明**：本节描述 Phase 2 的 API 设计。Phase 1 为静态站点，无 API 服务。
...
> **说明**：本节使用 axios、React Query 等库，为 Phase 2 技术栈示例。
...
我们采用分阶段推进策略，优先验证内容价值，再逐步添加复杂功能：
...
Phase 2: 用户系统（规划中）
├── 目标：提升粘性，积累用户数据
├── 触发条件：月活 > 5000 或用户强烈要求互动功能
├── 技术：接入 Supabase + NextAuth.js
├── 功能：登录、评分、评论、云同步
└── 优势：前端代码 100% 复用
...
| 功能 | 实现方式 | 理由 |
|------|---------|------|
| Skill 提交 | GitHub PR | 人工审核，保证质量 |
| 用户反馈 | 腾讯问卷 | 零开发成本 |
| 收藏功能 | LocalStorage | 即时可用，无需注册 |
| 阅读进度 | LocalStorage | 提升体验，无隐私顾虑 |
...
**关键优势**：
- 前端组件 100% 复用
- 只需修改 `lib/skills.ts` 等数据层
- 可并行运行，平滑迁移

#### Phase 3: 完整平台（未来规划）

待定，根据 Phase 2 数据决定方向。
...
| 项目 | 服务商 | 费用 | 说明 |
|------|--------|------|------|
| 域名 | 阿里云/腾讯云 | ¥50-100/年 | .com / .cn |
| 托管 | Vercel | ¥0 | 免费额度充足 |
| 数据 | GitHub | ¥0 | JSON 文件存储 |
| 分析 | Umami | ¥0 | 自托管或免费档 |
| **月度总计** | | **¥0-10** | 几乎零成本 |

> 注：Phase 1 可完全免费运行，适合验证市场需求。
...
| 数据库 (Supabase) | 500MB | Pro | ¥140 |
| 认证 (Clerk) | 5k MAU | 免费档 | ¥0 |
| 存储 (R2) | 10GB | - | ¥10 |
| **免费档总计** | | | **¥10** |
| **付费档总计** | | | **¥300** |

> 升级是可选的，根据实际用户量和功能需求决定。

### 6.3 扩展成本

| 阶段 | 用户量 | 月成本 | 技术方案 |
|------|--------|--------|---------|
| Phase 1 | < 10k | ¥0-10 | 静态站点 |
| Phase 2 | 1k-10k | ¥10-100 | 动态站点免费档 |
| Phase 2+ | 10k+ | ¥300+ | 动态站点付费档 |
...
| 模式 | 描述 | 预期收入 | 启动时间 |
|------|------|---------|---------|
| **精选推荐位** | Skill 置顶推广 ¥199/周 | ¥2000-8000/月 | Phase 1 后期 |
| **知识付费** | 小册/课程 ¥49-149 | ¥3000-10000/月 | Phase 2 |
| **企业定制** | 专属 Skill 开发 | ¥5000-50000/项目 | Phase 2 |
| **会员服务** | 高级功能 ¥29/月 | ¥1000-5000/月 | Phase 2 |
| **微信生态** | 付费社群、打赏 | ¥500-2000/月 | Phase 1 后期 |

### 7.3 与英文市场差异

| 维度 | 英文市场 | 中文市场 | 策略调整 |
|------|---------|---------|---------|
| 付费意愿 | 为工具付费习惯好 | 更愿为知识/服务付费 | 增加知识付费比重 |
| 获客渠道 | Product Hunt、Twitter | 微信群、知乎、B站 | 专注内容营销 |
| 企业客户 | SaaS 订阅为主 | 定制开发为主 | 提供定制服务 |
| 社区运营 | Discord 为主 | 微信群+公众号 | 建立微信生态 |

### 7.4 Phase 1 可启动的盈利方式

即使无用户系统，仍可盈利：

1. **云厂商联盟返佣**
   - 在 Skill 详情页添加"一键部署到阿里云/腾讯云"
   - 申请联盟账号，获取返佣链接
   - 预计：¥500-2000/月（需流量基础）
...
2. **精选推荐位（人工接单）**
   - 开发者通过表单提交推广申请
   - 人工审核、微信收款、手动上线
   - 预计：¥2000-5000/月（10-20 单）

3. **知识付费（小册）**
   - 编写《OpenClaw 实战指南》PDF
   - 通过小报童/公众号付费文章销售
   - 预计：¥1000-5000/月（20-100 本）
...
├── 技术社区
│   ├── V2EX 发布产品（精心准备的介绍帖）
│   ├── 掘金/知乎技术文章（OpenClaw 使用技巧）
│   └── 微信公众号分享（接入微信生态）
├── 社群渗透
│   ├── AI 工具交流群（分享有价值的内容）
│   ├── 开发者微信群（建立 3-5 个核心群）
│   └── Discord 中文频道（覆盖海外中文用户）
└── 内容营销
    ├── B站教程视频（操作演示类）
    ├── 小红书使用技巧（吸引非技术用户）
    └── 即刻产品动态（产品经理群体）
...
增长阶段（1000-10000 用户）
├── SEO 优化（百度/必应搜索排名）
├── 与 OpenClaw 官方合作（争取推荐位）
├── 开发者激励计划（投稿送周边）
└── 技术大会分享（QCon、ArchSummit）
```

**关键指标**：
- 微信公众号：500 关注可开通流量主
- 微信群：每群 200-400 人为佳
- SEO：目标"OpenClaw Skill"关键词前 3
...
- **Next.js 16** - App Router, SSG 静态生成
- **React 19** - UI 框架
- **TypeScript 5.x** - 类型安全
...
**关键优势**：
1. Phase 1 零成本运行，验证市场需求
2. 前端代码 100% 复用，平滑升级到 Phase 2
3. 数据驱动决策，再决定是否投入用户系统
